...
impl IntoClapMcpToolError for McpError {
	fn into_tool_error(self) -> ClapMcpToolError {
		ClapMcpToolError::text(gta_core::render_error(
			&self.0,
...
enum WorktreeAction {
	...
	Add {
		...
		#[arg(long, conflicts_with_all = ["force_branch", "detach"])]
		branch: Option<String>,
		/// Create or reset branch <name> and check it out.
		#[arg(long = "force-branch", conflicts_with = "detach")]
	...
	Remove {
		...
		#[arg(long)]
		path: PathBuf,
		/// Remove even if the worktree has changes; repeat to remove a locked worktree.
...
/// Map the clap `worktree` sub-command to the `gta-core` action. `--branch`/`--force-branch` collapse
/// to a branch name plus a force flag (git's `-B` is the force-create form).
