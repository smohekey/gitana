mod durability_target;
mod path_lock;

pub use durability_target::DurabilityTarget;
...
pub trait FileStore: Send + Sync {
	...
	/// Make the requested parts of every successful mutation completed before this call durable
	/// according to the backend's persistence guarantees.
	///
	/// Atomic writes keep readers from observing partial values, but do not by themselves promise
	/// that file contents, renames, or directory entries survive a power loss. Targets let a caller
	/// flush only the values and namespaces that form its logical publication boundary; a tree target
	/// remains available for explicit recovery barriers. The named paths must not be mutated
	/// concurrently. Returning success means those targets may be reported as durably committed.
	/// Backends without a persistence boundary, such as an in-memory store, implement this as a no-op.
