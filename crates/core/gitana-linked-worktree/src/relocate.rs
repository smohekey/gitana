...
mod native {
	...
	use crate::repo_id::reject_unsupported_repository_format;
	...
	pub async fn relocate(request: &RelocateRequest) -> Result<RelocateOutcome, RelocateError> {
		...
		// Fail closed on a repository format gitana does not fully understand — git's own
		// `check_repository_format` refuses to operate on such a repo (an unsupported version, or an unknown /
		// malformed-valued extension that could change the worktree or ref semantics the move touches), so
		// relocate refuses it before any effect, matching the forced-removal path's gate.
