...
async fn checkout_preserves_skip_worktree_and_does_not_materialise() {
	...
	make_repo(&work)
		.checkout(ObjectId::<Sha256>::from_hex(&other).unwrap(), false)
		.await
		.unwrap();
...
async fn checkout_excludes_new_paths_by_the_sparse_patterns() {
	...
	make_repo(&work)
		.checkout(ObjectId::<Sha256>::from_hex(&other_tree).unwrap(), false)
		.await
		.unwrap();
...
async fn force_checkout_removes_a_recreated_excluded_file() {
	...
	let head = git(&["-C", w, "rev-parse", "HEAD^{tree}"])
		...
		.to_owned();
	make_repo(&work)
		.checkout(ObjectId::<Sha256>::from_hex(&head).unwrap(), true)
		.await
		.unwrap();
