	...
	H: HashAlgorithm,
{
	...
	let dest_norm =
		GitPath::from_bytes(dest_norm).map_err(|_| WorktreeError::UnsafePathspec(dest.clone()))?;
	...
	if into_dir && !dest_is_dir {
		return Err(WorktreeError::MvDestinationNotDir(dest.clone()));
	}

	// Plan every move and validate it before touching the filesystem.
	let mut moves: Vec<(GitPath, GitPath)> = Vec::with_capacity(sources.len());
	let mut planned: BTreeSet<GitPath> = BTreeSet::new();
	for src in sources {
		let (src_norm, _) = normalize(src, prefix)?;
		let src_norm =
			GitPath::from_bytes(src_norm).map_err(|_| WorktreeError::UnsafePathspec(src.clone()))?;
		let is_file = tracked.contains(&src_norm);
		let is_dir = tracked.iter().any(|p| p.is_below(&src_norm));
		if !is_file && !is_dir {
			return Err(WorktreeError::MvSourceUntracked(src.clone()));
		}
		...
		if wt.work().lstat(&src_norm)?.is_none() {
			return Err(WorktreeError::MvBadSource(src.clone()));
		}

		let dst = if into_dir {
			let base = src_norm.file_name().expect("a source is not the root");
			dest_norm.join(&base)
		} else {
			dest_norm.clone()
		};
		...
		if let Some(matcher) = sparse.as_ref() {
			let lands_out = if is_file {
				...
				tracked
					.iter()
					.filter_map(|path| path.relative_to(&src_norm))
					.any(|rest| !matcher.includes(&dst.join_path(&rest)))
			};
			if lands_out {
				return Err(WorktreeError::SparsePathExcluded(dst.clone()));
			}
		}
		// Validate every index path the move will create under a directory source, before any
		// filesystem change.
		for path in &tracked {
			if let Some(rest) = path.relative_to(&src_norm) {
				let remapped = dst.join_path(&rest);
				validate_path(&remapped)?;
				// `reindex` refreshes stat data at every remapped path after the directory rename. A
				// UTF-8-only backend must reject a raw descendant now, while the source directory and
				// persisted index still agree, rather than after the rename has succeeded.
				wt.work().validate_path_representable(&remapped)?;
			}
		}

		if dst == src_norm || dst.is_below(&src_norm) {
			return Err(WorktreeError::MvIntoSelf(src.clone()));
		}
		if !force && wt.work().lstat(&dst)?.is_some() {
			return Err(WorktreeError::MvDestinationExists(dst.clone()));
		}
		if let Some(parent) = dst.parent()
			&& !parent.is_root()
			&& !matches!(wt.work().lstat(&parent)?, Some(meta) if meta.kind.is_dir())
		{
			return Err(WorktreeError::MvDestinationDirMissing(dst.clone()));
		}
		if !planned.insert(dst.clone()) {
			return Err(WorktreeError::MvDuplicateDestination(dst.clone()));
		}
	...
	H: HashAlgorithm,
{
	...
	let entries: Vec<IndexEntry<H>> = index
		.entries
		.iter()
		.filter(|e| e.stage == 0 && e.path.is_below(src))
		.cloned()
		.collect();
	for entry in entries {
		let tail = entry.path.relative_to(src).expect("selected descendant");
		let new_path = dst.join_path(&tail);
