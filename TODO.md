...
- [ ] Add submodule entry handling beyond tree mode representation. There is no `FileMode::Gitlink`: `crates/core/gitana-worktree/src/index.rs`'s `file_mode` maps mode `160000` through the default `Regular` (`100644`) case, so a gitlink recorded in the index cannot round-trip through `Index::tree_entries` into a commit — a later `gta commit` would write it as a blob pointing at a commit id (`git fsck` then reports "not a blob"). Consequently `checkout`/`switch`/a fast-forward `merge` that introduces a **gitlink** (a submodule) records a tree-incompatible index entry: an *out-of-cone* gitlink is accepted index-only (matching how `switch` on `main` already behaves — pre-dates the porcelain two-tree-merge migration), and an *in-cone* gitlink is refused pre-mutation by `merge_apply`'s blob preflight ("is not a blob"). Both are symptoms of the same missing end-to-end submodule support (index parse/serialize → `tree_entries`/commit → status/diff/ls-files → checkout). Full `FileMode::Gitlink` support is the fix; until then a branch introducing a submodule cannot be faithfully committed after checkout. Surfaced during the porcelain merge-unification review (codex); scoped out as a separate initiative.
