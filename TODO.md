...
- [ ] Carry forward staged index changes across `switch`/`reset --hard`/porcelain checkout, as git does. `WorkTree::checkout` (the `run` path) reconciles `(index, target)` treating the target tree as authoritative, so it silently **discards staged index divergences from HEAD** that git would preserve: a staged *new* file (`git add bar` then switch) is dropped, and a staged non-fast-forwardable change to an untouched path is lost — git carries both forward across a `switch`. **Pre-existing** (present on `main`, and independent of `core.ignoreCase` — reproduces with folding off) and orthogonal to the `is_ignored` unification, so deliberately left untouched by that work; surfaced during the `is_ignored` round-19 review. The `is_ignored` branch *did* fix the one axis it had regressed — a staged **case-rename** (`Foo`→`foo`) is now preserved via a three-way (HEAD-aware) reconciliation in both `run` (reads HEAD's tree, gated `fold && !force`) and `twoway_merge` (uses its `from` tree) — but the *general* staged carry-forward remains. Proper fix: make the `run` checkout a genuine two/three-tree merge against HEAD (like `twoway_merge` already is for fast-forwards), so `switch`/`reset --hard` and every porcelain checkout preserve unrelated staged work and refuse (not clobber) a conflicting staged change. This is the general form of the fast-forward-only follow-up noted under Merge And History Editing ("`switch` and `reset --hard` … could adopt `twoway_merge`"). Probe git 2.55 for the exact preserve-vs-refuse matrix (staged add, staged modify of an untouched path, staged modify of a touched path, staged delete) before pinning tests.
- [ ] Add attributes support where it affects working-tree behavior.
- [ ] Add line-ending normalization support.
- [x] Add sparse checkout support. `gta sparse-checkout init/set/add/list/disable/reapply` and `clone --sparse`, matching git: omit part of the working tree via the skip-worktree bit while keeping the full index/history. Both cone (directories) and non-cone (gitignore patterns) modes, stored git's way (`extensions.worktreeConfig` + per-worktree `core.sparseCheckout`/`core.sparseCheckoutCone` + `.git/info/sparse-checkout`), so stock git reads/operates in a gta-sparsified worktree. An omitted path is invisible to the working-tree commands as git makes it — `checkout`/`reset` keep it omitted (preserving the bit), `add`/`restore` skip it (never restaging or deleting its absent file), and a `checkout` under active sparsity recomputes each path from the current patterns (materialising only in-cone paths). Exposed on the `gitana:repo` wasm component too (`sparse-set`/`sparse-add`/`sparse-list`/`sparse-disable`/`sparse-reapply`). Oracle-tested against stock git 2.50.1. Sparse-*index* (collapsed directory entries) and partial clone (`--filter`) remain out of scope. **Follow-ups (documented, deferred):** (1) a `--sparse` escape to resolve an out-of-cone merge/cherry-pick/rebase *conflict* via `add` — git needs it too (plain `add` refuses an out-of-cone path, so gitana matches git's default refusal but lacks the escape hatch); (2) command-scope (`-c` / `GIT_CONFIG_COUNT`) precedence over `config.worktree` when reading `core.sparseCheckout` — a pre-existing gap shared with `status`'s `core.fileMode` read (see Remote And Protocol Parity's `config.worktree` note); (3) reconciling an *unchanged* recreated omitted file on a plain switch/fast-forward the way git's implicit reapply does (clean → re-omit, dirty → leave and warn) — cosmetic, no data loss.
