...
- [ ] Grow the component's WIT surface to the full repo-level op set (refs, rev-list/log, object reads, prune/gc) with richer per-op error variants; then two-descriptor `open` for linked worktrees.
- [ ] Retire `StdBackend`/`from_root` by porting `wasm-object-db` to `wasi:filesystem/preopens#get-directories` → `from_descriptor`.
