...
- [ ] Guard `merge` against silently deleting staged work. `merge::run` only refuses in-progress state and unmerged entries, then `checkout(merged_tree, false)`'s removal sweep drops index entries absent from the target — so a staged-but-uncommitted unrelated file is lost (git refuses). `cherry-pick` got a clean-index preflight (`conflict::index_tree(wt) == HEAD`); merge needs an equivalent, but more carefully: git's `merge` tolerates more local change than its `require_clean_work_tree`, so decide which staged states to refuse vs. allow (and add an oracle test against stock `git merge`).
- [ ] Add `revert`.
