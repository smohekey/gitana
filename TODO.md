...
- [ ] Add more complete `.gitignore` compatibility coverage. The shared glob engine now handles general backslash escapes (`\*`/`\?`/`\[`, an escaped trailing space) and POSIX character classes (`[[:digit:]]` etc.), landed with sparse-checkout (`d39bace8`). Remaining gaps: `.git/info/exclude`, the global excludes file, and `core.ignoreCase` folding in `.gitignore` matching (sparse-checkout folds, but `is_ignored` still matches case-sensitively).
