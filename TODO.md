...
- [x] Keep `gta` and `gta-mcp` command surfaces in lockstep as each command lands. Enforced by a
  `surface_parity` test that compares a normalized spec of both clap command trees — recursively,
  including root/global args, and per argument its required-ness, action (arity), allowed values,
  defaults, and groups — normalizing the intended positional-vs-named presentation, native
  `--checkout`/`--merge` flags vs. the MCP strategy enum, and MCP-only serving flags. The MCP tool
  schema deliberately omits `ls-files -z` because raw NUL-delimited bytes cannot be returned as a
  JSON string; direct `gta-mcp ls-files -z` remains byte-exact, while the MCP tool forces reversible
  quoting.
...
- [x] Resolve remote-command config against the *worktree-effective* stack. `fetch`, `pull`, `push`,
  and `trust sync` now acquire the retained setup/config capabilities and call
  `git_config::for_worktree_at`, layering `<gitdir>/config.worktree` when
  `extensions.worktreeConfig` is enabled. Per-worktree `remote.*`, `http.*`, credential, SSH,
  protocol, rewrite, and signing-policy overrides therefore participate in the same effective stack
  as the rest of the command, without reopening config through an ambient path. `ec906b7d`.
...
- [x] Preserve repository-controlled Git paths as exact bytes across object, repository, index,
  worktree, porcelain, CLI, MCP, and component boundaries. The dedicated `gitana-path` crate now
  distinguishes repository paths, components, pathspecs, and tree-entry names instead of passing
  them through `String`; Unix command arguments and working-tree names round-trip non-UTF-8 bytes,
  while filesystems that cannot represent a name reject it during a full pre-mutation preflight.
  Human CLI output retains Git-compatible quoting; MCP uses injective reversible result rendering
  plus tagged base64url inputs for raw path/revision bytes; WIT exposes explicit `utf8`/`bytes`
  variants and rejects an unrepresentable raw name only at the WASI filesystem boundary. Covered by
  `git_non_utf8_paths.rs`, MCP tool-call regressions, component host tests, and raw-byte unit tests.
  This does not change remote URL syntax: non-UTF-8 SSH remote paths remain separately deferred.
  `68d0a12a`.
- [x] Expand pathspec support beyond simple files, directories, and `.`. Full glob and magic-prefix pathspecs across `add`/`rm`/`restore`/`reset`/`checkout` (`810caf2b`): default globbing (`*`/`?`/`[]` cross `/`, `**` not special) and `:(glob)` FNM_PATHNAME globstar mode with full `**` parity; POSIX classes / ranges (incl. descending) / escapes; `:(exclude)`/`:!`/`:^` negatives, `:/`/`:(top)`, `:(icase)`, `:(literal)`, `:(glob)`, short-magic signature rejection. Matching = wildmatch ∪ literal exact/leading-dir pass; `rm -r` only for a pure directory expansion; sparse out-of-cone paths get git's deferred advice. Matcher memoized (no exponential blow-up). Staged/removed index is **byte-identical to git 2.50.1** across two differential fuzzers + 16 codex rounds. **Deliberate documented divergence** (on `WorkTree::add`'s doc): git's exclude-pathspec wart (a non-root positive + any negative suppresses untracked staging) is *not* replicated — gitana stages them. **Follow-ups (deferred):** `add`-ing untracked files under a directory whose literal name contains a glob metacharacter; a pathspec argument for `ls-files`. `:(attr:...)` tracked under Attributes; in-worktree absolute pathspecs still rejected.
- [x] `gta add -f`/`--force` and git's ignored-path advisory, rendered as git's exact blocks. The ignored-path advisory is a pathspec-*level* diagnostic (matching git): EVERY element — positive OR negative — whose literal or glob base is or lies under an *existing* ignored path is reported, collapsed to where the rule matched (`ign/new` → `ign`, `root.log` → `root.log`), lexicographically sorted, independent of exclusion/tracked/existence — so `add ign/tracked`, `add ign/new :!.`, and `add :!ign/x` all advise, while a broad `.`/glob or a negative naming a non-ignored path stays silent. Staging still refuses only *untracked* ignored content (a tracked mod beneath an ignored path stages); `-f`/`--force` bypasses it everywhere, including broad/`<dir>`/glob traversals. As part of this the **sparse-checkout advisory was reworked to git's exact 7-line block** (`advice.updateSparsePath`-aware) via a shared `PathspecAdvisory { sparse, ignored }` error, and when an ignored and a sparse advisory both apply, `gta` prints both. Both advisories exit 1 after the stageable work is saved; `advice.addIgnoredFile`/`advice.updateSparsePath` are read+validated before staging. Threaded through `WorkTree::add`, the `gta`/`gta-mcp` CLIs, and the `gitana:repo` wasm component (WIT `add` gains a `force` param; package bumped `0.11.0`→`0.12.0`). Byte-for-byte vs git 2.50.1 in `crates/cli/gta/tests/git_add.rs` for the ignored advisory (all cases) and a single-kind sparse advisory. **Documented divergence (deferred):** git renders the sparse advisory as *separate per-pass blocks* — tracked skip-worktree pathspecs (argument order, dups) and discovered/untracked out-of-cone paths (sorted, deduped) as distinct blocks, tracked first (`add out/a out/new` → two blocks); gitana renders one merged block, so an `add` mixing tracked+untracked out-of-cone paths (or repeating/multi untracked ones) may order/dedup them differently. The ignored-path advisory is unaffected. (5 codex rounds; the sparse multi-block structure surfaced in round 5 and was deliberately left as a documented divergence.)
- [x] Give `gta ls-files` a pathspec argument and git's selection surface. `ls-files` now takes variadic pathspecs (reusing the glob/magic `PathspecSet`) plus `-c/--cached`, `-s/--stage`, `-o/--others`, `-m/--modified`, `-d/--deleted`, `--exclude-standard`, `--error-unmatch`, `-z`, and `--full-name`, threaded through `gta`/`gta-mcp` (no wasm surface). Output matches git byte-for-byte: cwd-relative paths (`--full-name` for repo-relative, implicit-subtree scoping in a subdir), C-style `core.quotePath` quoting, the per-selector duplication model (a conflicted path once per stage; `-c -m -d` prints a modified+deleted path three times), and unmerged/sparse/assume-valid/skip-worktree handling. `-o --exclude-standard` honours all standard excludes — per-directory `.gitignore`, `.git/info/exclude`, and `core.excludesFile` (with `~`/XDG default, relative resolved against the worktree toplevel), plus `core.ignoreCase` folding — and treats an unreadable ignore file as non-fatal but a directory at one as fatal, like git. Untracked embedded repositories collapse to an opaque `dir/` only when `.git` is a valid repo (validated `HEAD` + `objects`/`refs` dirs); unreadable dirs/files never abort; special files (FIFO/socket/device) are skipped; `core.fileMode`/`core.quotePath`/`core.ignoreCase`/`core.symlinks` resolve from the full config stack via `LsFilesConfig`. For `-m`/`-d`, tracked-membership honours `core.ignoreCase` (case-folded lookup), a `core.symlinks=false` symlink *placeholder* (a plain file holding the link target) is modified only when its content diverges, and a submodule *gitlink* (`160000`) is reported modified only when its checked-out `HEAD` differs from the recorded commit (resolved for ordinary and linked worktrees; dirty state does not count). `crates/cli/gta/tests/git_ls_files.rs` diffs 26 cases against git 2.50.1; 6 codex rounds (6/7/7/11/3/3 findings, all confirmed + fixed). **Documented divergences (deliberate, all pathological):** (1) `-o` does not prune pathspec-excluded directories before walking — a *perf* difference only, output/exit identical to git; (2) `--error-unmatch` reproduces git's dedup for exact-file pathspecs (plain, `:(literal)`, and top-magic `:/`/`:(top)`) but does not replicate git's arcane ps_matched-across-selector-passes bookkeeping, so an exotic error-unmatch combined with `:(exclude)` and multiple selectors can differ in duplicate-line output and, for an untracked-only self-excluded positive (`-o --error-unmatch u :!u`), in exit status; (3) a *relative* `$XDG_CONFIG_HOME` for the default excludes file is resolved against the process directory, not the repository. **NOTE:** the case-fold, `.git/info/exclude`, and `core.excludesFile` handling has since been lifted into the shared `status`/`checkout`/`add` engine too (see the `.gitignore` compatibility item below), and gitlink handling is now shared by status/diff/checkout/add/rm rather than being `ls-files`-only. The remaining `ls-files`-only compatibility is its explicit `core.symlinks=false` placeholder handling; the shared status engine can still report such a Unix plain-file placeholder as modified.
- [x] Add more complete `.gitignore` compatibility coverage. The shared glob engine now handles general backslash escapes (`\*`/`\?`/`\[`, an escaped trailing space) and POSIX character classes (`[[:digit:]]` etc.), landed with sparse-checkout (`d39bace8`). `ls-files --exclude-standard` learned `.git/info/exclude`, the global excludes file, and `core.ignoreCase` folding first; those three are now **also** lifted into the shared `is_ignored` path used by `status`, `checkout`'s untracked-overwrite guard, and `add`/`rm`'s untracked walk (and the ignored-path advisory). `core.ignoreCase` and `.git/info/exclude` are resolved inside the sandboxed worktree crate (the effective config, and the `.git` store) via `crate::excludes::standard_excludes`; the global `core.excludesFile` — outside the worktree — is resolved by the frontend (`gta-core::excludes::resolve_excludes_file`, shared with `ls-files`) and passed in, so it is native-only. `status` also folds tracked-vs-untracked membership under `core.ignoreCase`, matching `ls-files`. The wasm `status`/`add`/`checkout` ops keep the unchanged WIT surface: the component resolves `core.ignoreCase` + `.git/info/exclude` internally and passes `None` for the (sandbox-absent) global excludes file. Oracle-tested against git 2.55: `status` vs `git status` with `core.ignoreCase=true`, a `.git/info/exclude` rule, and a `core.excludesFile` rule (engine differential in `git_status.rs`, end-to-end `gta status` in `gta/tests/git_status.rs`). `checkout` is fully `core.ignoreCase`-aware: its index/target/tracked/stray comparisons all fold, so a `Foo`→`foo` case-rename across a checkout keeps the file and rewrites only the index entry to the target case (both non-force and force; byte-checked vs git 2.55), rather than deleting the re-created file. `--force` skips the overwrite *protection* but not config validation — a malformed `core.ignoreCase` or a directory `.git/info/exclude`/`core.excludesFile` is fatal even under force, as it is for git — and `switch`/`switch -c` validate and update the working tree *before* publishing the branch or moving `HEAD`, so an aborted checkout (a broken config, or an in-the-way untracked file) leaves no ref behind. `add` with no pathspec is git's "nothing added" no-op that validates `core.ignoreCase` but reads no exclude file, and `add -f` bypasses exclude-file loading (a directory `info/exclude` is not fatal there) while still validating `core.ignoreCase`. **Documented scope boundaries (deliberate):** (1) the *tracked-membership* case-fold is applied in `status` (user-visible `??` parity) but **not** in `add`'s guard, where a folded mismatch only ever makes it treat a disk file as untracked and *skip* it — the safe direction; (2) `twoway_merge` (the fast-forward path, driven only by `merge`) is fully `core.ignoreCase`-aware too — it compares `from`/`to`/`staged` by fold-key and re-cases the working-tree entry, so a case-rename fast-forward applies like git — and it plus every internal porcelain checkout pass `None` for the global excludes file (still honouring `core.ignoreCase` + `info/exclude`); (3) the wasm component has no source for a global excludes file (no `$HOME`/XDG), so a host-injected one is out of scope; (4) the frontend resolves `core.excludesFile` *eagerly* for every `status`/`switch`/`reset --hard`, whereas git reads the exclude files *lazily* — only when a checkout actually updates the tree. This matters only for a **directory-valued** `core.excludesFile`/`info/exclude` (the sole fatal case): a `switch -c <name>` whose start point is the current `HEAD` (a pure branch-create with no tree change) reads nothing in git and so succeeds, but gta resolves/validates the exclude file up front and errors. A pathological configuration; matching git's exact create-at-HEAD read-laziness is not worth the frontend/worktree-boundary plumbing.
- [x] Carry forward staged index changes across a non-force `switch`, as git does. A non-force `switch` is now git's **two-tree merge** (`read-tree -m -u`) from the current HEAD's tree to the target (`WorkTree::checkout_merge` → the `merge_apply` engine, alongside a new `CheckoutMode<H> { Reset, Overlay, Merge { head } }`): it carries a staged *new* file, a staged non-fast-forwardable change to an untouched path, and unstaged work across the switch, refusing (not clobbering) a real conflict — where the old authoritative reconciliation silently discarded them. Byte-identical to git 2.55 across a large oracle suite: the preserve-vs-refuse matrix (staged add/modify-untouched/modify-touched/delete + unstaged), the full 34-cell directory/file (D/F) conflict matrix (worktree-state-dependent, incl. staged D/F collisions and preserved-vs-discarded reconciliation), three refuse-parity fixes (a `switch -f`/non-force during an in-progress merge/cherry-pick/revert/rebase — incl. a **stock-git** `rebase-merge`/`rebase-apply` rebase — and a sparse out-of-cone staged-deletion the target would overwrite), sparse-checkout reconciliation of carried out-of-cone entries (clean→omit+preserve-blob, dirty→leave, recreated-at-omitted, satisfied), a tracked-**file→directory** replacement (refuses rather than silently deleting the directory's ignored contents), the lock-ordering **lost-update** race (index lock now taken before the index read), full `core.ignoreCase` fold coverage (staged case-rename/addition/deletion, colliding additions, D/F folded ancestors), and cancellation-safety (an RAII `IndexLock` guard: atomic commit-and-release, plus fail-closed lock retention once a worktree mutation has begun). Unborn/missing/unmerged index and delayed branch-publication handled. `--force` keeps the authoritative reset (correctly discarding, and recoverable even when the current tip is corrupt); `reset --hard` is likewise force. **Verify-porcelain follow-up — DONE (PR #13):** the non-force *porcelain* checkouts (cherry-pick/revert/merge-final via the shared conflict writer, rebase replay + base move) now use `Merge` (`checkout_merge`), and the merge fast-forward folded `twoway_merge` in (deleted). `Overlay` is retained only for the *headless* non-force checkouts (the wasm component's `checkout`, a fresh linked-worktree's materialise) — fully eliminating it is not correct for those. 16 codex review rounds (r1–r15 fixes + the r16 confirming pass), each finding probed vs git 2.55 (two turned out to be codex false positives that would have introduced divergences). **Deliberate documented divergence:** the fast-forward now aborts on the *first* conflicting path (git and the retired `twoway_merge` list *all* overwritten paths) — the single-path `Err` was chosen for the unified engine; the ignored/index-affecting outcome is byte-identical, only the diagnostic lists one path. **Gitlink/submodule follow-up — DONE:** see the next two items.
- [ ] Add attributes support where it affects working-tree behavior.
- [ ] Add line-ending normalization support.
- [x] Add sparse checkout support. `gta sparse-checkout init/set/add/list/disable/reapply` and `clone --sparse`, matching git: omit part of the working tree via the skip-worktree bit while keeping the full index/history. Both cone (directories) and non-cone (gitignore patterns) modes, stored git's way (`extensions.worktreeConfig` + per-worktree `core.sparseCheckout`/`core.sparseCheckoutCone` + `.git/info/sparse-checkout`), so stock git reads/operates in a gta-sparsified worktree. An omitted path is invisible to the working-tree commands as git makes it — `checkout`/`reset` keep it omitted (preserving the bit), `add`/`restore` skip it (never restaging or deleting its absent file), and a `checkout` under active sparsity recomputes each path from the current patterns (materialising only in-cone paths). Exposed on the `gitana:repo` wasm component too (`sparse-set`/`sparse-add`/`sparse-list`/`sparse-disable`/`sparse-reapply`). Oracle-tested against stock git 2.50.1. Sparse-*index* (collapsed directory entries) and partial clone (`--filter`) remain out of scope — and gitana's index parser maps the sparse-directory mode `040000` to a regular blob (`file_mode`), so a **stock-git sparse index is mis-parsed everywhere** (status, add, checkout). In particular the non-force `switch` two-tree merge would compare a placeholder `out/` entry against the recursively-expanded HEAD/target trees and could treat deleted-below-`out/` paths as satisfied or produce an overlapping `out/`+`out/c` index — so switching inside a repo whose index stock git collapsed is unsupported and can misrepresent the index. Full sparse-index support (expand-on-read, or detect + reject) is the separate initiative that would close this gitana-wide. **Follow-ups (documented, deferred):** (1) a `--sparse` escape to resolve an out-of-cone merge/cherry-pick/rebase *conflict* via `add` — git needs it too (plain `add` refuses an out-of-cone path, so gitana matches git's default refusal but lacks the escape hatch); (2) command-scope (`-c` / `GIT_CONFIG_COUNT`) precedence over `config.worktree` when reading `core.sparseCheckout` — a pre-existing gap shared with `status`'s `core.fileMode` read (see Remote And Protocol Parity's `config.worktree` note); (3) reconciling an *unchanged* recreated omitted file on a plain switch/fast-forward the way git's implicit reapply does (clean → re-omit, dirty → leave and warn) — cosmetic, no data loss.
- [x] Add faithful gitlink entry handling across the index, commit, status, diff, ls-files, checkout,
  switch, restore, add, rm, sparse checkout, worktree removal, and three-way conflicts.
  `FileMode::Gitlink` now round-trips mode `160000` without treating its commit id as a blob;
  checkouts materialize/reuse mount directories without descending into initialized submodules;
  status/diff compare the mounted module's `HEAD` (including linked-worktree module stores); and
  divergent pointers record `160000` conflict stages while preserving populated mounts. The
  stock-Git oracle matrix in `git_submodule.rs` covers clean/moved/absent/retyped mounts, D/F and
  case-fold collisions, null OIDs, special nodes, sparse paths, populated conflicts, add/rm
  opacity, and retained checkout reuse. `e28ed2bf` plus the subsequent gitlink hardening series.
- [x] Add capability-safe submodule consumer operations and recursive clone integration.
  `gta submodule` supports status, init, update (checkout/merge strategy, remote/no-fetch, shallow
  policy, recursion), deinit, set-branch, set-url, and recursive sync; clone can publish and then
  initialize selected nested modules. Repository/config/mount identities are revalidated across
  waits, mutations are serialized, interrupted publication has durable recovery, credentials are
  redacted, relative URLs and linked worktrees are handled, and recursive operations preserve
  deterministic level order. The merge strategy supports fast-forward, true signed merges, and
  materialized conflicts inside existing modules; new/reattached modules remain detached exact
  checkouts. Covered by the 220-case `git_submodule_commands.rs` matrix and the core state-machine
  tests. `a6bfda58` through `fa473137`.
...
- [x] Add regression tests for SHA-256 repository config compatibility. Repository config parsing,
  detection, initialization, stock-Git interoperability, descriptor-backed component opening, and
  worktree/remote/submodule operations all exercise SHA-256 repositories; tests probe the installed
  stock Git and skip only when it lacks SHA-256 support.
...
- [x] Define reliable cross-target compile gates. CI builds the production WASI guest root with
  `cargo build -p gitana-repo-component --target wasm32-wasip2`; a workspace-wide WASI
  `--all-targets` build is intentionally excluded because it selects native CLI and host-only
  test/dev dependency graphs that are not part of the component. Native Windows CI runs
  `cargo check --workspace --all-targets` on MSVC plus the storage durability tests; locally, the
  broader Windows GNU graph compiles with
  `cargo zigbuild --workspace --all-targets --target x86_64-pc-windows-gnu`. Keep cross-compilation
  evidence distinct from native runtime validation.
