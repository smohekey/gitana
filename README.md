...
- `gta submodule status [--recursive]` / `init` /
  ...
  nested module, retaining that root for an explicit no-path retry if a child fails. The retry honors
  the effective activation set across all config layers. Literal directory-form selectors match
  submodule gitlinks as directories; directory-only wildcard selectors retain Git's no-match
  behavior for gitlinks. `clone --recurse-submodules --shallow-submodules` requests depth one for
  every selected module and descendant; `submodule update --depth N` applies an absolute positive
  depth to both new and existing module repositories. As with top-level clone, initial native-local
